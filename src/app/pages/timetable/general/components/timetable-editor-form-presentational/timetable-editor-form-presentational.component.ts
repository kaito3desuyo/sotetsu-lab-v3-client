      ...
      return this.fb.group({
        ...
        times: this.fb.array(
          this.stations.map(station =>
            this.generateTime(
              station,
              find(trip.times, time => time.stationId === station.id)
            )
          )
        )
      });
  ...
  generateTime(station: IStation, time?: ITime): FormGroup {
    if (time) {
      return this.fb.group({
        id: [time.id],
        stopType: [
          time.pickupType === 1 && time.dropoffType === 1 ? 'passing' : 'stop',
          Validators.required
        ],
        ...
        arrivalTime: [
          {
            value: time.arrivalTime
              ? moment(time.arrivalTime, 'HH:mm:ss').format('HH:mm')
              : null,
            disabled: false
          }
        ],
        departureTime: [
          {
            value: time.departureTime
              ? moment(time.departureTime, 'HH:mm:ss').format('HH:mm')
              : null,
            disabled: false
          }
        ]
